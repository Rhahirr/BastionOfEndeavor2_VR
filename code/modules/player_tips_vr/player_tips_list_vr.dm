	...
	switch(choice)
		if("Общий совет")
			...
			return pick(
				...
				prob(35); "[info] Столкнулись с чем-то похожим на баг? Даже если сомневаетесь, не стесняйтесь доложить об нём администраторам!",
				...
				prob(30); "[info] Нужна помощь с созданием персонажа или совет насчет отыгрыша? В нашем Дискорде Вам будут рады помочь в канале SS13-Обучение!",
		...
		if("Игровой процесс")
			...
			return pick(
				...
				prob(35); "[info] Вы можете спрятать свою идентификационную карту в КПК. Если после этого переместить КПК в слот для карты, это позволяет освободить место на поясе под что-то другое. Больее того, если нажать на КПК и перетащить его мышкой на экран с игрой, Вы сможете пользоваться им без необходимости сперва взять его в руку!",
				prob(35); "[info] В органах панели Vore доступны различные модификаторы: с их помощью можно заглушить речь своей жертвы, чтобы не выдавать её присутствие, или отключить её датчики, чтобы не медицинский отдел не переживал за её состояние.",
				prob(35); "[info] При любых травмах лучше сразу же обратиться в медицинский отдел - маленькие ожоги и раны могут привести к инфекции, что затрудняет их лечение. Если на смене нет врачей, Вы можете найти в баре или туалетах настенные НаноМеды, в которых находятся мазь для ожогов и бинты для ушибов.",
				...
				prob(25); "[info] Духовка на кухне позволяет складывать сразу несколько ингредиентов за раз, если сперва достать из неё поднос и выложить всё на него. Большинство рецептов работают именно так, и это применимо в том числе и для гриля и фритюрницы!",
		...
		if("Ролевой отыгрыш")
			...
			return pick(
				...
				prob(40); "[info] Постарайтесь не создавать персонажа, который знает всё на свете. Отыгрывать персонажа, которые владеет только несколькими навыками, гораздо интереснее. Ничего, если где-то что-то сломалось и некому больше чинить - если это не Ваша обязанность, то Вам не обязательно этим заниматься.",
				...
				prob(35); "[info] Если Вы замедлитесь при встрече с другим игроком, у Вас будет больше возможностей на отыгрыш! Возможно, другой игрок будет печатать Вам ответ, поэтому не убегайте мимо него, иначе всё пропустите!",
				prob(50); "[info] Не получается найти ролевую сцену? Самый лучший способ их найти - инициировать самостоятельно и быть активным! Как правило, другим будет проще с Вами взаимодействовать, если Вы двигаетесь и чем-то занимаетесь. Не стесняйтесь разговаривать с людьми, потому что с Вами мало кто заговорит, если Вы молча сидите за столом. Те, кто ищут сцены, обычно сперва хотят посмотреть, как Вы отыгрываете, прежде чем инициировать.",
				prob(25); "[info] После использования механики обнимашек и прочих взаиомдействий рекомендуется подождать пару мгновений. Возможно, другой игрок захочет написать Вам ответ, поэтому не убегайте сразу же, иначе всё пропустите!",
				...
				prob(10); "[info] При вылазках во Врата или на космических локациях хорошей идеей может быть отправить факс на Центральное Командование, использовать глагол Молиться или даже просто запросить Помощь админа через F1. Этим Вы говорите администраторам, что происходит что-то интересное, а они могут помочь разнообразить Вашу вылазку чем-то интересным!"
				)
		...
		if("Сеттинг и лор")
			...
			return pick(
				...
				prob(75); "[info] В отличие от других серверов, синтеты (киборги и позитроники), как правило, обладают теми же правами, что и люди. Дроны, функционирующие на основе реалистичного ИИ, в этом плане чуть сложнее. https://bastionofendeavor.ddns.net/wiki/Синтеты",
				...
				prob(5); "[info] Позитронные разумы ценятся за их надежность, самодостаточность и сопротивление к вмешательствам извне. Способности к запоминанию информации и скорости её обработки у таких разумов сравнима с человеческими.",
