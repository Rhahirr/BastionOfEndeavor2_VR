...
/obj/item/organ/external/proc/robo_repair(var/repair_amount, var/damage_type, var/damage_desc, obj/item/tool, mob/living/user)
	...
	if(brute_dam + burn_dam >= min_broken_damage) //VOREStation Edit - Makes robotic limb damage scalable
		...
		*/
		to_chat(user, "<span class='danger'>Повреждения на [pcase_ru(src)] слишком серьёзные, чтобы их можно было заделать внешне.</span>")
		// End of Bastion of Endeavor Translation
		return 0
	...
	if(damage_desc)
		if(user == src.owner)
			...
			*/
			user.visible_message("<b>[cap_ru(user)]</b> [damage_desc] на [verb_ru(src, "сво;ём;ей;ём;их;")] [pcase_ru(src)] с помощью [gcase_ru(tool)].")
			// End of Bastion of Endeavor Translation
		else
			/* Bastion of Endeavor Translation
			user.visible_message("<b>\The [user]</b> patches [damage_desc] on [owner]'s [src.name] with [tool].")
			*/
			user.visible_message("<b>[cap_ru(user)]</b> [damage_desc] на [pcase_ru(src)] [gcase_ru(owner)] с помощью [gcase_ru(tool)].")
			// End of Bastion of Endeavor Translation
...
/obj/item/organ/external/proc/createwound(var/type = CUT, var/damage)
	...
	if((damage > 15) && (type != BURN) && (local_damage > 30) && prob(damage) && (robotic < ORGAN_ROBOT) && !(species.flags & NO_BLOOD))
		...
		*/
		owner.custom_pain("Вы почуствовали, как что-то порвалось внутри [verb_ru(src, "Ваш;ем;ей;ем;их;")] [pcase_ru(src)]!", 50)
		// End of Bastion of Endeavor Translation
			...
			if(compatible_wounds.len)
				...
				if(prob(25))
					if(robotic >= ORGAN_ROBOT)
						...
						*/
						owner.visible_message("<span class='danger'>Рана на [pcase_ru(src)] [gcase_ru(owner)] расширилась, порвавшись с тошнотворным звуком.</span>",\
						"<span class='danger'>Рана на [verb_ru(src, "Ваш;ем;ей;ем;их;")] [pcase_ru(src)] расширилась, порвавшись с тошнотворным звуком.</span>",\
	...
	switch(disintegrate)
		if(DROPLIMB_EDGE)
			if(!clean)
				...
				*/
				var/gore_sound = "[(robotic >= ORGAN_ROBOT) ? "скрежет металла" : "ripping tendons and flesh"]"
				owner.visible_message(
					...
					"<span class='danger'>Вы слышите ужасающий [gore_sound].</span>")
				// End of Bastion of Endeavor Translation
		if(DROPLIMB_BURN)
			if(cannot_gib)
				return
		...
		if(DROPLIMB_BLUNT)
			...
			*/
			var/gore = "[(robotic >= ORGAN_ROBOT) ? "": ", разбрызгав повсюду кровь и мясо"]"
		...
		if(organ_can_feel_pain() && !isbelly(owner.loc) && !isliving(owner.loc))
			...
			*/
			owner.visible_message(\
				...
				"<span class='danger'>Вы слышите, как что-то громко хрустнуло.</span>")
			owner.emote("scream")
			// End of Bastion of Endeavor Translation
